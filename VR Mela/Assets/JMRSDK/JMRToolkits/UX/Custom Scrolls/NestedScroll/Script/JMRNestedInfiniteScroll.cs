...
namespace JMRSDK.Toolkit
{
    ...
    /// </summary>
    public class JMRNestedInfiniteScroll : MonoBehaviour, IDropHandler
    {
        ...
        class CacheScrollElementData
        {
            public GameObject[] objects;
            public RectTransform[] objectTransforms;
        }

        private Dictionary<string, CacheScrollElementData> objectCache;
        ...
        void Awake()
        {
            poolCount = poolItemCount = 0;
            initialized = finishedSpawning = false;
            objectCache = new Dictionary<string, CacheScrollElementData>();
        ...
        int poolCount, poolItemCount;
        IEnumerator SpawnPoolObjects()
        {
            GameObject clone;
            RectTransform rect;
            for (int i = poolCount; i < poolPrefabs.Length; i++)// GameObject obj in poolPrefabs)
            {

                CacheScrollElementData cachedData = new CacheScrollElementData();
                if (objectCache.ContainsKey(poolPrefabs[i].name))
                {
                    cachedData.objects = objectCache[poolPrefabs[i].name].objects;
                    cachedData.objectTransforms = objectCache[poolPrefabs[i].name].objectTransforms;
                }
                else
                {
                    cachedData.objects = new GameObject[fillCount];
                    cachedData.objectTransforms = new RectTransform[fillCount];
                }
                for (int j = poolItemCount; j < fillCount; j++)
                {
                    yield return new WaitForEndOfFrame();
                    clone = Instantiate(poolPrefabs[i], Vector3.zero, Quaternion.identity);
                    ...
                    cachedData.objects[j] = clone;
                    cachedData.objectTransforms[j] = rect;
                    if (!clone.gameObject.GetComponent<JMRNestedInfiniteScroll>())
                        clone.SetActive(false);

                    if (objectCache.ContainsKey(poolPrefabs[i].name))
                    {
                        objectCache[poolPrefabs[i].name] = cachedData;
                    }
                    else
                    {
                        objectCache.Add(poolPrefabs[i].name, cachedData);
                    }
                    poolItemCount = j + 1;
                }
                poolCount = i + 1;
                //Debug.LogError("Instantiating " + poolPrefabs[i].name);
            }
        ...
        void UpdateVertical()
        {
            ...
            int border = ItemSpacing + _heights[0];
            int step = (int)((_topPosition - TopPadding) / border);
            ...
            if (position < 0 || position >= _count || _previousPosition == position)
            {
                return;
            }
            if (position > _previousPosition)
            {
                ...
                if (index < _count)
                {
                    ...
                    if (IsPullBottom && index == _count - PullValue)
                        OnPull?.Invoke(Direction.Bottom);
            ...
            else
            {
                ...
                if (position < _count - fillCount)
                {
                    ...
                    // if (IsPullBottom && position == PullValue)
                    //OnPull?.Invoke(Direction.Top);
        ...
        void UpdateHorizontal()
        {
            ...
            int border = ItemSpacing + _widths[0];
            int step = (int)((_leftPosition - LeftPadding) / border);
            if (step != _saveStepPosition)
            {
                _saveStepPosition = step;
            }
        ...
        {
            if (!initialized || !finishedSpawning)
                return;
        ...
        public void InitData(int count)
        {
            if (Type == 0)
            {
                StartCoroutine(InitVertical(count));
            }
            else
            {
                StartCoroutine(InitHorizontal(count));
            }
        }
        ...
        IEnumerator InitVertical(int count)
        {
            ...
            yield return StartCoroutine(CreateViews());
            _previousPosition = 0;
            _count = count;
        ...
        /// </summary>
        /// <param name="count">Item count</param>
        IEnumerator InitHorizontal(int count)
        {
            while (!finishedSpawning)
            {
                yield return new WaitForEndOfFrame();
        ...
        void ApplyDataToVertical(int count, int newCount, Direction direction)
        {
            ...
            _content.sizeDelta = new Vector2(_content.sizeDelta.x, height);
            Vector2 pos = _content.anchoredPosition;
            if (false)//direction == Direction.Top)
            {
                float y = 0f;
        ...
        void ApplyDataToHorizontal(int count, int newCount, Direction direction)
        {
            ...
            float width = CalcSizesPositions(_count);
            _content.sizeDelta = new Vector2(width, _content.sizeDelta.y);
            Vector2 pos = _content.anchoredPosition;
            if (false)//direction == Direction.Left)
            {
                float x = 0f;
                for (int i = 0; i < newCount; i++)
                {
        ...
        IEnumerator MoveTo(Direction direction)
        {
            ...
            else
            {
                ...
                end = (direction != Direction.Right) ? 0f : 1f;
            ...
            while (timer <= 1f)
            {
                ...
                if (Type == 0)
                {
                    ...
                    _scroll.velocity = new Vector2(0f, (direction != Direction.Bottom) ? -speed : speed);
                ...
                else
                {
                    ...
                    _scroll.velocity = new Vector2((direction != Direction.Right) ? speed : -speed, 0f);
            ...
            if (Type == 0)
            {
                _scroll.velocity = new Vector2(0f, (direction != Direction.Bottom) ? -SCROLL_SPEED : SCROLL_SPEED);
                //_scroll.velocity = new Vector2(0f, (direction == Direction.Top) ? -SCROLL_SPEED : SCROLL_SPEED);
            }
            else
            {
                _scroll.velocity = new Vector2((direction != Direction.Right) ? SCROLL_SPEED : -SCROLL_SPEED, 0f);
        ...
        /// Create views
        /// </summary>
        IEnumerator CreateViews()
        {
            if (Type == 0)
            {
                yield return StartCoroutine(CreateViewsVertical());
            }
            else
            {
                yield return StartCoroutine(CreateViewsHorizontal());
            }
        }
        ...
        /// </summary>
        IEnumerator CreateViewsVertical()
        {
            ...
            if (objectCache.ContainsKey(Prefab.name))
            {
                _views = objectCache[Prefab.name].objects;
                _rects = objectCache[Prefab.name].objectTransforms;
            }
            else
            {
                ...
                for (int i = 0; i < fillCount; i++)
                {
                    ...
                    _views[i] = clone;
                    _rects[i] = rect;
                    //Debug.LogError("Spawning views vert" + clone.name);

                    yield return waitForEndOfFrame;
                }

                objectCache.Add(Prefab.name, new CacheScrollElementData() { objects = _views, objectTransforms = _rects });
            }
        ...
        /// </summary>
        IEnumerator CreateViewsHorizontal()
        {
            if (currentPrefab == Prefab)
            {
                yield return null;
            ...
            else
            {
                ...
                for (int i = 0; i < fillCount; i++)
                {
                    ...
                    _views[i] = clone;
                    _rects[i] = rect;
                    //Debug.LogError("Spawning views horiz" + clone.name);
                    yield return waitForEndOfFrame;
                }
